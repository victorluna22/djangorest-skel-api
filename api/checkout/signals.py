...
def post_save_order_receiver(sender, order, **kwargs):
	...
	if order.status == ORDER_AUTHORIZED and order.payment_type.credit_ipill and not order.operations.all():
	...
	if order.status == ORDER_AUTHORIZED and order.situation == OPENED:
		...
		message = 'Seu pedido #%06d foi entregue com sucesso. Obrigado por usar o iPill!' % order.id
